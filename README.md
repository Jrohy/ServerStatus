...
# 更新说明：
* 2018.02.26, 优化负载显示。现在如果是SSR等能监听到tcp6的，连接数大于0则会显示出相应的连接数，显示组合为一分钟负载|tcp6连接数，否则只显示一分钟负载。如果是SSR却显示连接数失败，则可能没安装netstat,手动安装 apt-get(yum) install net-tools即可
